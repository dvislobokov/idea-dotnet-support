...
Статус: **история, не действует (2026-09-22).** Этапы 0–5 и completion из этапа 6 были сделаны на платформенном DAP-клиенте и проверены вживую
2026-09-21, затем отладчик перенесён на свой DAP-клиент (пакет `debugger`, план — `DAP_PLAN.md`): модуля `intellij.platform.dap` нет
в IntelliJ IDEA Community и её форках. Content-модуля `io.github.dotnetsupport.dap` больше нет, промпт ниже устарел. Ценное здесь — «Журнал»:
находки про адаптер, аргументы `launch`, сборку перед отладкой, attach и отладку тестов действуют и для своего клиента.
...
### Этап 6, completion в выражениях отладчика (2026-09-21) — проверено UI-роботом
У адаптера запроса `completions` нет (в его ROADMAP — пункт 4.4, после Roslyn), поэтому имена берутся у остановленной программы:
на отдельно стоящем имени — локальные переменные и параметры кадра (`scopes` + `variables`) и члены `this`; после `значение.` — именованные дети значения,
найденного через `evaluate`. Дополняются только простые цепочки имён (`person.Friend.`, `this.`, `a?.`): для `Make().` пришлось бы выполнять вызов
во время набора. `lang/CSharpDebugCompletion` (чистая функция контекста), в модуле — `DotNetEditorsProvider` (редакторы Evaluate / watch / условия точки
останова стали фрагментами C#: подсветка и метка для контрибьютора) и `DotNetExpressionCompletionContributor` (`order="first"`, `stopHere()`: слова
остальных контрибьюторов там только шум). Когда адаптер получит `completions` — добавить его первым источником. Проверено роботом: пустое поле →
локальные, `per` → `person`, `person.` → члены, `person.Friend.N`, `Greet(person).` → ничего.
